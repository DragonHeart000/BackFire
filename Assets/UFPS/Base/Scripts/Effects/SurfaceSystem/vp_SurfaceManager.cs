...
public class vp_SurfaceManager : MonoBehaviour
{
	...
	public static Texture GetTerrainTexture(Collider col, Vector3 position)
	{
        ...
        var terrainTextureID = GetDominantTerrainTexture(position, terrain);
	...
	protected List<Texture> GetLevelTerrainTextures()
	{
		...
		for (int v = 0; v < terrains.Length; v++)
		{
			...
			if (terrains[v].terrainData == null)
				continue;
#if UNITY_2018_3_OR_NEWER
            if (terrains[v].terrainData.terrainLayers == null)
                continue;
            if (terrains[v].terrainData.terrainLayers.Length < 1)
                continue;
            for (int s = 0; s < terrains[v].terrainData.terrainLayers.Length; s++) {
                if (terrains[v].terrainData.terrainLayers[s].diffuseTexture == null)
                    continue;
                if (textures.Contains(terrains[v].terrainData.terrainLayers[s].diffuseTexture))
                    continue;
                textures.Add(terrains[v].terrainData.terrainLayers[s].diffuseTexture);
            }
#else
